...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT unique_id, role, password, blocked FROM users WHERE unique_id =? OR email =?");
    $stmt->bind_param("ss", $identifier, $identifier);
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0) {
        $stmt->bind_result($db_unique_id, $role, $db_password, $blocked);
        $stmt->fetch();

        if ($blocked == 1) {
            echo "<script>alert('Your account has been blocked!'); window.location.href='login.php';</script>";
            exit();
        }

        if ($role === 'Admin') {  
            // Admins use plain text passwords
            if ($password === $db_password) {  
                $_SESSION['user_id'] = $db_unique_id;
                $_SESSION['role'] = $role;

                $check_profile_stmt = $conn->prepare("SELECT * FROM admin_profiles WHERE user_id =?");
                $check_profile_stmt->bind_param("s", $db_unique_id);
                $check_profile_stmt->execute();
                $check_profile_stmt->store_result();

                if ($check_profile_stmt->num_rows == 0) {
            ...
            // Volunteers & Restaurants use hashed passwords
            if (password_verify($password, $db_password)) {  
                $_SESSION['user_id'] = $db_unique_id;
                $_SESSION['role'] = $role;

                if (strpos($db_unique_id, '002') === 0) {
                    header("Location: volunteer_dashboard.php");
                } elseif (strpos($db_unique_id, '003') === 0) {
                    header("Location: restaurant_dashboard.php");
                } else {
                    echo "<script>alert('Invalid Unique ID!'); window.location.href='login.php';</script>";
                }
                exit();
            } else {
                echo "<script>alert('Incorrect password!'); window.location.href='login.php';</script>";
...
<!-- New Neumorphic Login Form -->
<div class="login-container">
    <h2>Login</h2>
    <form action="login.php" method="POST">
        <input type="text" name="identifier" placeholder="Enter your Unique ID or Email" required>
