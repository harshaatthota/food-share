...
// Fetch restaurants
$query_restaurants = "SELECT unique_id, email, role, blocked FROM users WHERE role = 'Restaurant' AND role != 'Admin'";
$result_restaurants = $conn->query($query_restaurants);
?>
...
tr:hover {
    background: rgba(26, 194, 26, 0.3); /* Light green tint matching header */
    transition: background 0.3s ease-in-out;
}
...
<p><h1>Manage Restaurants</h1></p>
    <table>
        ...
        <tbody>
            <?php while ($row = $result_restaurants->fetch_assoc()): ?>
            <tr>
                <td><?= $row['unique_id'] ?></td>
                <td><?= $row['email'] ?></td>
                <td><?= $row['role'] ?></td>
                ...
                <td>
    <?php if ($row['blocked']): ?>
        <a href="toggle_block.php?email=<?= $row['email'] ?>&action=unblock&role=Restaurant">Unblock</a>
    <?php else: ?>
        <a href="toggle_block.php?email=<?= $row['email'] ?>&action=block&role=Restaurant">Block</a>
    <?php endif; ?>
</td>
